...
\begin{solution}
  \begin{subprob}
  \item $B = C_n$ as blocks can be cycles and the greatest clique that can be in $G$ is $K_3$ which is the same as $C_3$.
  \item Let $v$ be a vertex that breaks the outerplanar property. There must be a cycle around $v$, $v$ must be connected to two parts of the cycle (otherwise $v$ is on a branch that can just be drawn on the outside). Since $v$ is connected to two vertices in the cycle there are two cycles that go through $v$ who share at least $3$ vertices with eachother, $v$ and each of the vertices connected to $v$. This contradicts the definition of $G$ and thus $v$ can not be in $G$.
  \end{subprob}
\end{solution}
...
\begin{solution}
  For an interval graph to have a claw, there needs to be an interval $I$ that overlaps three other intervals $\{i_1, i_2, i_3\}$ and no $i$ can overlap. For this to be possible, $I$ must be larger than one $i$ to have some room left for the other $i$. This contradicts the definition of a unit interval graph as all intervals must have the same size.
\end{solution}
