...
\begin{solution}
Since vertices 1-5 form an odd cycle, the chromatic number of the Petersen graph $P$
is $3 \leq \chi(P)$. Furthermore, by Brook's theorem, we have that $\chi(P) \leq
\Delta(P) = 3$, so $\chi(P) = 3$.
\end{solution}

\begin{solution}
Since vertices 1-5 form an odd cycle, the chromatic number of the Grötzsch graph
...
colours to colour vertices 1-5, and there is a 1-1 correspondence between the
colours of 1-5 and 6-10, you need 3 colours to colour vertices 6-10. This is a
contradiction since vertex 11 is connected to each of vertices 6-10, and thus
...
\begin{solution}
For every colour $1$ and $2$, there must be a exist a pair of vertices $v, u \in
...
\begin{solution}
  ...
  means that there is at least one independent set $S$ of vertices (vertices that
  share the same colour), with at least $\frac{n}{k}$ vertices.
  ...
  \textbf{Base case:}
  A graph consisting of a single vertex, and its complement, can be coloured
  with 1 colour. $1 + 1 \leq n + 1$ = 2
